...
- 클로저를 통해서 할 수 있는 것 : 캡슐화 및 정보를 은닉할 수 있음
...
- 얕은 복사: 참조에 의한 할당, 해당 데이터의 참조 값(메모리 주소)를 전달하는 방식으로 하나의 데이터를 공유 함
- 깊은 복사: 깊은 복사는 저는 스프레드 연산자를 통해서 배열의 값을 깊은 복사해 본 적이 있습니다. 깊은 복사는 메모리 주소를 참조하는 것이 아닌 별개의 메모리 주소를 할당받습니다.
  단 주의 해야할 점은 1depth까지만 허용됩니다.
...
- 자바스크립트의 변수는 선언 -> 초기화 -> 할당의 과정을 거치는데 var의 경우 변수 선언과 초기화가 함께 진행되므로 변수 객체 등록과 동시에 메모리도 할당받는다.
  원래 하단에서 변수를 선언하고 위에서 호출을 하면서 참조 오류 같은 오류가 발생해야 하는데 var로 선언했을 때는 실행 컨텍스트를 통해서 최 상단에 끌어올려지기 때문에 undefined가 출력된다.
