...
## 웹 페이지의 성능을 고려할 때 HTML 파일에서 `<script>`태그를 어디에 위치시키는 게 좋을지 설명해 주세요.

페이지 렌더링을 차단하지 않고 빠르게 페이지를 표시하기 위해서는 `<body> `태그 맨 아래에 위치 시키는 걸 권장합니다. 그 이유는 `<head>`태그 내에 `<script>`태그가 존재한다면 스크립트의 로드가 완료될 때까지 페이지가 표시되지 않을 수 있기 때문입니다.

## 자바스크립트에서 원시(primitive) 타입과 참조(reference) 타입에 대해서 설명해 주세요.

원시 타입의 종류에는 string, number, bigint, boolean, null, undefined, symbol 등이 존재하고 불변(immutable) 하며, 복사될 때 값 자체가 복사됩니다.

참조 타입의 종류에는 array, function, object, date 등이 존재하고 객체(object)를 나타내며, 복잡한 데이터 구조를 다루는 데 사용됩니다. 참조 타입은 가변(mutable) 하며, 해당 객체 또는 배열의 메모리 위치를 참조하며, 변수 간의 할당 시 메모리 주소가 복사됩니다. 따라서 변수 간에 값을 수정하면 모든 참조가 해당 값을 반영하게 됩니다.

## 자바스크립트에서 클로져(Closures)에 대해서 설명해주세요.

클로져란? 내부 함수에서 외부 함수에 맥락에 접근할 수 있는 것을 의미합니다. 그렇다면 일반적으로 함수가 선언된 렉시컬 스코프(Lexical Scope) 환경에서 전역 렉시컬 스코프(Lexical Scope) 환경에 변수를 가리키는 것 모두 클로져 일까요? 아닙니다! 클로져는 외부 함수 안에 내부 함수가 묶여있는 것처럼 폐쇄적인 느낌을 주는 것이 클로져입니다.
...
## HTTP 프로토콜 메서드 GET과 POST의 차이 점에 대해서 설명해 주세요.
...
## 얕은 복사(Shallow Copy)와 깊은 복사(Deep Copy)에 대해서 설명해 주세요.

얕은 복사란? 얕은 복사는 원본 데이터의 구조를 복사하지만 내부 요소들은 원본 데이터의 참조를 공유하는 방식입니다. 다시 말해, 복사본의 내부 구조는 새로 생성되지만 내부의 객체나 배열 등의 요소들은 원본과 같은 메모리 주소를 참조하게 됩니다. 이로 인해 얕은 복사를 수행한 두 개의 객체는 내부 요소 중 하나를 수정하면 다른 객체에도 영향을 미칩니다.

깊은 복사란? 깊은 복사는 원본 데이터의 구조와 내부 요소 모두를 완전히 새로운 메모리 공간에 복사하는 방식입니다. 이를 통해 복사본과 원본은 서로 독립적인 메모리 공간을 가지게 되므로 한쪽에서 내부 요소를 수정하더라도 다른 쪽에는 영향을 주지 않습니다. 하지만 깊은 복사는 객체나 배열이 복잡한 중첩 구조를 가지고 있는 경우 메모리 사용량이 증가하고 복사 과정이 복잡해질 수 있습니다. 제가 경험한 건 spread(...Object) 스프레드 연산자를 통해서 객체를 복사한 경험이 있었는데 복사한 객체의 1depth 요소를 변경할 때는 원본 객체에 영향을 주지 않았지만 2depth 이상의 요소를 변경하면 원본 객체에도 영향을 주었습니다.
...
- 추가적으로 DeppCopy를 할 수있는 방법은 JSON.parse(JSON.stringify(object));를 통해서도 깊은 복사를 할 수 있습니다.
